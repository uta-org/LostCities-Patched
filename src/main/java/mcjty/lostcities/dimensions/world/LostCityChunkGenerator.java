...
public class LostCityChunkGenerator implements IChunkGenerator, ICommonGeneratorProvider {
    ...
    public LostCityChunkGenerator(World world, long seed) {
        ...
        if(LostCitiesDebug.debug) System.out.println("Registering chunk generator!");
    ...
    public Chunk generateChunk(int chunkX, int chunkZ) {
        return generateChunk(chunkX, chunkZ, false);
    }

    public Chunk generateChunk(int chunkX, int chunkZ, boolean forceCityChunk) {
        LostCityProfile profile = BuildingInfo.getProfile(chunkX, chunkZ, this);

        terrainGenerator.setupChars(profile);
        boolean isCity = BuildingInfo.isCity(chunkX, chunkZ, this) || forceCityChunk;

        ChunkPrimer chunkprimer = getChunkPrimer(chunkX, chunkZ, isCity);

        try {
            terrainGenerator.generate(chunkX, chunkZ, chunkprimer, forceCityChunk);
        } catch (Exception e) {
