...
public class Highway {
     ...
     */
    public static int getXHighwayLevel(int chunkX, int chunkZ, ICommonGeneratorProvider provider, LostCityProfile profile) {
        return getHighwayLevel(provider, profile, Highway.xHighwayLevelCache, cp -> hasXHighway(cp, profile), Orientation.X, new ChunkCoord(provider.getDimensionId(), chunkX, chunkZ));
    }
     ...
     * Returns 0 or 1 if there is a highway (at that city level) going through this chunk.
     */
    public static int getZHighwayLevel(int chunkX, int chunkZ, ICommonGeneratorProvider provider, LostCityProfile profile) {
        return getHighwayLevel(provider, profile, Highway.zHighwayLevelCache, cp -> hasZHighway(cp, profile), Orientation.Z, new ChunkCoord(provider.getDimensionId(), chunkX, chunkZ));
    }

    private static int getHighwayLevel(ICommonGeneratorProvider provider, LostCityProfile profile, Map<ChunkCoord, Integer> cache, Function<ChunkCoord, Boolean> hasHighway, Orientation orientation, ChunkCoord cp) {
